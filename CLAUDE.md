...
**`/config/mapeo` › «Mapeo balance/cliente» es UNA sola vista del PUC completo del cliente** (todo `cuentas_cliente`, acumulado de todas sus cargas, ordenado por código como árbol) **editable en cualquier nivel**. El nivel que se muestra es la longitud real del código (N4, N6, N8, N10, N12…): la BD colapsa todo lo de 8+ dígitos en `nivel = 8` (`nivelPorCodigo`), pero el código completo se conserva. Editar una cuenta de 4/6 dígitos fija la regla del GRUPO (`manual`, propaga por prefijo); editar una **auxiliar** (más de 6 dígitos) deja SIEMPRE una excepción `manual_cuenta` —`editarMapeoCliente` lo fuerza aunque la fila fuera `manual` de grupo, porque una auxiliar `manual` gana la elección del grupo y arrastra a sus hermanas—. El filtro **«Revisar»** (`detectarAnomaliasMapeo`: auxiliares cuyo mapeo difiere de la regla de su grupo sin excepción declarada) ofrece **«Resolver»** en la fila: «Alinear con el grupo» (`alinearMapeoConGrupo`, copia la regla gobernante con su MISMO origen vía `planAlinearConGrupo`/`elegirReglaGrupo`, nunca fabrica una fila manual bajo un grupo automático) o «Mantener como excepción» (`declararExcepcionMapeo` → `manual_cuenta` al 100%). Como editar la memoria no toca balances ya cargados, **«Reaplicar a balances cargados»** (`reaplicarMapeoBalancesCliente` en `balance.ts`) re-homologa todos los balances no congelados del cliente, uno por transacción, con el mismo núcleo (`rehomologarBalance`) que el botón «Re-homologar» del detalle de un balance.
...
### Balance por tercero (captura automática, sin pantalla propia)
