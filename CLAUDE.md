...
- **Subtotales del archivo = CONTROL, nunca dato** (`src/lib/modulos/subtotales.ts`, puro): los archivos pueden traer o no filas de subtotal por clasificador. `transformarModulo` las detecta al leer (`detectarSubtotales`: rótulo «Total/Subtotal», fila sin detalle —referencia/documento/tercero vacíos—, negrita, y **aritmética**: la fila vale la Σ del bloque contiguo del mismo clasificador, arriba o abajo) y las marca `tipoFila: "total"` con `motivo_tipo_fila` («subtotal:rotulo,aritmetica», «gran_total:…»). El consolidado sale SIEMPRE de los movimientos (`consolidarPorClasificador` y `esImputable` excluyen todo lo que no sea `movimiento`); el borrador muestra el panel **Control de subtotales** (`controlSubtotales`: Σ movimientos del bloque vs. subtotal del archivo, recalculado en vivo al omitir/rescatar) y lo exporta en la hoja «Control subtotales». No bloquea la carga. Falsos positivos/negativos se corrigen por fila («Incluir» / «Marcar subtotal», `tipoFilaForzado: "total"`) y la preferencia `SpecModulo.subtotales` (`auto` | `rotulo` | `nunca` | `manual`) se memoriza en el perfil de formato del cliente (modal de carga y `/config/perfiles-carga/[fuente]`). El modo `manual` es la salida cuando ninguna heurística sirve: el usuario elige la COLUMNA del archivo que marca esas filas (`subtotalesColumna`, 1-based, puede ser una columna NO mapeada a ningún rol) y, opcionalmente, el TEXTO que la marca (`subtotalesTexto`; vacío = basta con que la celda traiga algo, `coincideMarcaSubtotal` compara normalizado). En ese modo NADA se marca por cuenta propia —ni el gran total— salvo lo que señale la columna; sin columna elegida la carga se rechaza en vez de degradar a «ninguna». Sin subtotales en el archivo no hay panel y el comportamiento es el de antes.
