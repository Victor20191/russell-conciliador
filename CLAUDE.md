...
- `src/lib/ia/proveedor-balance.ts` — compuerta central doble: Gemini exige `NODE_ENV=development` + `BALANCE_AI_DEV_SELECTOR=true`; en cualquier otro contexto fuerza Anthropic. Sin la clave del proveedor activo, un perfil guardado igual habilita la carga determinista (si no, plantilla limpia).
