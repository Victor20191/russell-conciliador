...
- **Cruce contable** (`cruce-contable.ts`, `cruce-tercero.ts`): compara el consolidado del módulo contra el balance por cuenta Russell de 4 díg.; las diferencias se documentan con marcas de auditoría (sección siguiente). El **cruce por tercero** (pestaña propia) está habilitado en TODOS los módulos vía `crucePorTercero` del descriptor y compara clave a clave contra `balance_tercero_detalle` — la clave es el rol `rolCruceTercero` (default `tercero`; Nómina cruza por `cedula`, con `rolNombreCruceTercero: "empleado"`). El lado contable se deduplica con `filasEfectivasTercero` (las filas «propias» de cuentas con detalle no suman doble).
