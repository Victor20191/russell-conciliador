...
- **A. IMPORT_READY** — las filas de detalle sugeridas (cuentas de movimiento; la plataforma resuelve cuáles son las hojas por prefijo, salvo que indiques una columna marcadora), con `CUENTA, NOMBRE_CUENTA, SALDO_INICIAL, DEBITOS, CREDITOS, SALDO` (más `centro` si aplica). La cabecera va aparte.
