...
10. **Detalle por tercero.** Distingue las columnas de identidad: `columnas.tercero` corresponde al **documento** (Identificación, NIT, cédula, Código SN); `columnas.nombreTercero` corresponde al **nombre o razón social** (Nombre tercero, Nombre NIT, Nombre SN, Razón social o Tercero si contiene nombres). Si el archivo tiene «Identificación» y «Tercero», mapea la primera como documento y la segunda como nombre; no confundas el nombre del tercero con `columnas.nombre`, que es el nombre de la cuenta. Usa `0` cuando un rol no exista. Si documento y nombre vienen juntos, mapea esa celda en `tercero` y deja `nombreTercero=0`; si solo hay nombres, usa `tercero=0` y mapea `nombreTercero`. Un documento vacío con nombre presente sigue siendo un tercero. Si el archivo también trae una fila consolidada del mismo código sin identidad ni centro de costo, marca `importable=true` y deja que la plataforma use la fila consolidada y omita el desglose para no doble-contar. Si NO existe fila consolidada y solo hay detalle por tercero, marca `agregarPorTercero=true`; el código agrupará por cuenta sumando los cuatro importes. **Dentro de ese desglose es posible que un mismo código traiga el mismo NIT de tercero con importes idénticos, incluido el saldo final; eso NO es un duplicado (suelen ser centros de costo o unidades distintos) y no debes omitirlo: súmalo igual que las demás filas del mismo tercero con importes diferentes, para mostrar un solo registro por cuenta con el saldo total.** **Nunca uses el NIT del tercero como clave corporativa.**
