...
6. **Fila de detalle (cuenta imputable) — prioriza el nivel más profundo.** La regla base es la **longitud** del código: define `reglaDetalle.tipo="longitud"` y `longitudMin` = la longitud **mínima inclusiva** de una cuenta de detalle en ese archivo. **Prefiere el nivel auxiliar (8 dígitos):** si el archivo trae cuentas de **8 o más dígitos**, usa `longitudMin=8` (las subcuentas de 6 dígitos quedan como padres y se excluyen, sin doble conteo). **Si NO hay cuentas de nivel 8, baja al nivel 6** (subcuenta): usa `longitudMin=6` (no exijas «> 6»: un balance de 6 dígitos es válido y mapea al estándar). Para auxiliares de **7 dígitos** usa `longitudMin=7`. Si existe una columna marcadora de detalle (`Rompimiento=Cuenta`, `cueclasificacion=I`, `indicador=1`, `Movimiento_Diario` o equivalente), prefiérela: `reglaDetalle.tipo="columna"`, con `columna` y `valor`. Excluye cuentas padre aunque tengan códigos artificiales largos (p. ej. 15 dígitos). **Respaldo determinista del código:** si pides `longitudMin=8` pero el archivo no contiene ninguna cuenta de 8+ dígitos, la plataforma cae automáticamente a nivel 6 para no dejar el cargue vacío.
