...
async function leerLibroExcel(data: ArrayBuffer): Promise<LecturaExcel> {
  ...
  for await (const ws of wb) {
    ...
    for await (const row of ws) {
      ...
      const values = crudos.map((_, j) => valorCeldaExcel(row.getCell(j + 1)));
  ...
  for await (const ws of wb) {
    ...
    for await (const row of ws) {
      ...
      if (row.number === filaBuscada) {
        const celda = row.getCell(columnaBuscada);
        // Texto que el lector no alcanzó a resolver (ver `leerLibroExcel`): el llamador
        // repite la consulta con SheetJS.
        if (esTextoCompartidoSinResolver(celda.value)) throw new LecturaExcelIncompleta();
        return { hojaExiste: true, filaExiste: true, valor: valorCeldaExcel(celda) };
      }
      if (row.number > filaBuscada) {
