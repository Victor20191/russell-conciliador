...
import { evaluarAdopcion, type CambioNovedadContexto } from "@/lib/auditoria/reporte-ejecutivo/adopcion";
import AuditoriaTabs from "../auditoria-tabs";
import { ReporteEjecutivoClient, type KpisIniciales, type VersionOpcion } from "./reporte-ejecutivo-client";
...
export default async function AuditoriaAdopcionPage() {
  await requirePermiso("auditoria:reporte_ejecutivo");
  const [accesosAuth] = await Promise.all([authorizePermiso("auditoria:accesos")]);
  ...
  const [eventosRaw, versiones, clientes] = await Promise.all([
    ...
    prisma.platformVersion.findMany({
      ...
      include: {
        changes: { orderBy: [{ order: "asc" }, { id: "asc" }], select: { id: true, moduleKey: true, title: true, type: true, description: true, route: true, howTo: true, example: true, featureStatus: true } },
      },
    }),
  ...
  return (
    <div>
      <PageHeader
        title="Auditoría"
        subtitle="Indicadores permanentes de uso de la plataforma, adopción de novedades y generación del reporte ejecutivo para el cliente."
      />
      <AuditoriaTabs canAccesos={accesosAuth.ok} canReporteEjecutivo />
