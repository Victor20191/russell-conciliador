...
export default async function DianPage() {
  const forms = await prisma.dianForm.findMany({
    include: { periods: { orderBy: { periodKey: "desc" } } },
  });
  ...
  return (
    <div>
      <PageHeader
        title="Impuestos · DIAN"
        subtitle="Conciliación de formatos tributarios declarados vs. contabilidad"
      />
          ...
          <Card key={f.id}>
            <div className="flex items-center gap-3 border-b border-ink-100 px-4 py-3">
              ...
              <div className="flex-1">
                <h2 className="text-[13px] font-semibold text-ink-800">{f.name}</h2>
                <div className="text-[11.5px] text-ink-500">
                  <span className="font-mono">{f.code}</span> · {f.periodicity}
                </div>
              </div>
            </div>
              ...
              {f.periods.map((p) => (
                <div key={p.id} className="flex items-center justify-between px-4 py-2.5">
                  <div>
                    ...
                    {p.filed && (
                      <div className="text-[11px] text-ink-400">Presentado: {p.filed}</div>
                    )}
                  </div>
                  <Chip label={p.status} tone={tone(p.status)} />
                </div>
              ))}
            </div>
          </Card>
