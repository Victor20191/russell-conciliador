...
function CargarModal({
  ...
  const analizar = (hoja?: string) => {
    ...
    if (!clienteId) { notifyError("Selecciona el cliente."); return; }
    startAnalizar(async () => {
      ...
      footer={
        fase === "archivo" ? (
          <>
            ...
            <button type="button" disabled={!tieneArchivo || !clienteId || analizando} onClick={() => analizar()} className="rounded-md bg-navy-700 px-3 py-1.5 text-[12.5px] font-semibold text-white hover:bg-navy-600 disabled:opacity-60">
      ...
      {fase === "archivo" ? (
        <div className="flex flex-col gap-3.5 text-[12.5px]">
          <label className="flex flex-col gap-1">
            ...
            <select value={clienteId} onChange={(e) => setClienteId(e.target.value)} className="rounded-md border border-ink-200 bg-white px-2.5 py-2 text-ink-700 outline-none focus:border-blue-400">
        ...
        <div className="flex flex-col gap-3.5 text-[12.5px]">
          ...
          {(analisis?.hojas?.length ?? 0) > 1 && (
            <label className="flex items-center gap-2">
              ...
              <select value={spec?.hoja ?? ""} onChange={(e) => analizar(e.target.value)} className="rounded-md border border-ink-200 bg-white px-2 py-1 text-ink-700 outline-none focus:border-blue-400">
                {analisis?.hojas?.map((h) => <option key={h} value={h}>{h}</option>)}
              </select>
              <span className="text-[11px] text-ink-400">{analisis?.totalFilas} filas</span>
            </label>
          )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="flex flex-col gap-1">
              ...
              <input type="number" min={1} value={spec?.filaEncabezado ?? 1} onChange={(e) => setEnc(Math.max(1, Number(e.target.value) || 1))} className="rounded-md border border-ink-200 bg-white px-2.5 py-1.5 tabular-nums text-ink-700 outline-none focus:border-blue-400" />
            </label>
            <label className="flex flex-col gap-1">
              <span className="text-[11px] font-medium text-ink-600">Primera fila de datos</span>
              <input type="number" min={1} value={spec?.primeraFilaDatos ?? 2} onChange={(e) => setDat(Math.max(1, Number(e.target.value) || 1))} className="rounded-md border border-ink-200 bg-white px-2.5 py-1.5 tabular-nums text-ink-700 outline-none focus:border-blue-400" />
            </label>
          </div>
          ...
          <div className="rounded-md border border-ink-150">
            ...
            <div className="flex flex-col divide-y divide-ink-100">
              {roles.map((rc) => {
                ...
                return (
                  <div key={rc.nombre} className="grid grid-cols-[minmax(0,150px)_minmax(0,1fr)] items-center gap-3 px-3 py-2">
                    <span className="text-[12px] font-medium text-ink-700">
                      {rc.etiqueta}{rc.requerido && <span className="text-err-700"> *</span>}
                      {rc.nombre === clasificadorRol && <span className="ml-1 rounded bg-blue-100 px-1 text-[9.5px] font-semibold text-blue-700">clasifica</span>}
                    </span>
                    <div className="flex items-center gap-2">
                      <select value={spec?.columnas[rc.nombre] ?? 0} onChange={(e) => setCol(rc.nombre, Number(e.target.value))} className="min-w-0 flex-1 rounded-md border border-ink-200 bg-white px-2 py-1 text-[12px] text-ink-700 outline-none focus:border-blue-400">
                        <option value={0}>— sin mapear —</option>
                        {opcionesColumna().map((o) => <option key={o.index1} value={o.index1}>{o.label}</option>)}
                      </select>
                      <span className="w-32 shrink-0 truncate text-[11px] text-ink-400" title={muestras.join(" · ")}>{muestras.filter(Boolean).slice(0, 2).join(" · ") || "—"}</span>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="flex flex-col gap-2 rounded-md border border-ink-150 bg-ink-50 px-3 py-2.5">
            <label className="flex flex-col gap-1">
              ...
              <select value={modo} onChange={(e) => setModo(e.target.value as typeof modo)} className="rounded-md border border-ink-200 bg-white px-2 py-1.5 text-[12px] text-ink-700 outline-none focus:border-blue-400">
            ...
            {modo === "seccion" && (
              <label className="flex flex-col gap-1">
                ...
                <select value={spec?.seccionColumnaVaciaRol ?? "descripcion"} onChange={(e) => setSeccionRol(e.target.value)} className="rounded-md border border-ink-200 bg-white px-2 py-1.5 text-[12px] text-ink-700 outline-none focus:border-blue-400">
          ...
          <label className="flex w-1/2 flex-col gap-1">
            ...
            <input type="month" value={mes} onChange={(e) => setMes(e.target.value)} className="rounded-md border border-ink-200 bg-white px-2.5 py-1.5 text-ink-700 outline-none focus:border-blue-400" />
          </label>
        </div>
      )}
    </Modal>
