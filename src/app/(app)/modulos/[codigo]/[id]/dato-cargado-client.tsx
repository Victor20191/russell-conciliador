...
function ConsolidadoTab({
  ...
  const [valores, setValores] = useState<Record<string, string>>(() =>
    Object.fromEntries(consolidado.map((c) => {
  ...
  const nombrePorCuenta = new Map(cuentas.map((c) => [c.codigo, c.nombre]));

  const guardar = (clasificador: string) => {
    const cuenta4 = (valores[clasificador] ?? "").replace(/\D/g, "").slice(0, 4);
    ...
    startGuardar(async () => {
      ...
      if (r.ok) { notifySuccess(r.message ?? "Consolidación guardada."); router.refresh(); }
    ...
    <Card className="p-0">
      <div className="overflow-x-auto">
        <table className="w-full text-[12.5px]">
          ...
          <tbody>
            {consolidado.map((c) => {
              const cuentaActual = (valores[c.clasificador] ?? "").replace(/\D/g, "").slice(0, 4);
                  ...
                  <td className="px-3 py-2">
                    {puedeEditar ? (
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          type="button"
                          disabled={guardandoClave === c.clasificador}
                          onClick={() => guardar(c.clasificador)}
                          className="rounded-md border border-ok-500 bg-ok-100/40 px-2 py-1 text-[11px] font-semibold text-ok-700 hover:bg-ok-100 disabled:opacity-60"
                        >
                          {guardandoClave === c.clasificador ? "…" : "Guardar"}
                        </button>
                        {nombre ? (
