...
import { consolidarPucCliente, type CuentaPucCliente } from "@/lib/balance/catalogo-puc-cliente";
import { Prisma } from "@/generated/prisma/client";
import { authorizePermiso, requirePermiso } from "@/lib/rbac";
import { alcanceLecturaUsuario } from "@/lib/rbac/contexto";
import { PageHeader } from "@/components/ui";
import type { StdAccount } from "@/lib/balance/tipos-mapeo";
import MapeoClienteClient from "./mapeo-cliente-client";

export default async function MapeoCuentasClientePage({ searchParams }: { searchParams: Promise<{ cliente?: string }> }) {
  ...
  const [sp, alc] = await Promise.all([searchParams, alcanceLecturaUsuario()]);

  // No depende del cliente elegido: se inicia antes del selector para solaparla
  // con la consulta de balances.
  ...
  // escritura (la action revalida el alcance real).
  ...
  const [puedeMapear, accounts, standard, historicas, originales] = await Promise.all([
    clienteId
    ...
    clienteId
      ? prisma.$queryRaw<Array<{ id: number; code: string; name: string; cuenta6Russell: string | null; coincidencia: Prisma.Decimal | null; actualizadoEn: Date; fechaBalance: Date; balanceId: number; periodo: string; version: string }>>(Prisma.sql`
          SELECT DISTINCT ON (d.cuenta_8)
            ...
            e.id AS "balanceId", e.creado_en AS "fechaBalance", e.periodo, e.version
      ...
      : Promise.resolve([]),
    clienteId
      ? prisma.$queryRaw<Array<{ id: number; code: string; name: string; tipoFila: "movimiento" | "agrupadora"; fechaBalance: Date; periodo: string; version: string }>>(Prisma.sql`
          SELECT DISTINCT ON (cuenta->>'codigo') e.id, cuenta->>'codigo' AS code,
            cuenta->>'nombre' AS name, e.periodo, e.version, e.creado_en AS "fechaBalance",
            CASE
              WHEN EXISTS (
                SELECT 1 FROM balance_prueba_detalle d
                WHERE d.encabezado_id = e.id AND d.cuenta_8 = cuenta->>'codigo'
              ) THEN 'movimiento'
              WHEN cuenta->>'tipo_fila' IN ('movimiento', 'agrupadora') THEN cuenta->>'tipo_fila'
              ELSE 'agrupadora'
            END AS "tipoFila"
  ...
  const acc: CuentaPucCliente[] = consolidarPucCliente(
    ...
    historicas.map((a) => ({ ...a, fechaBalance: a.fechaBalance.toISOString(), tipoFila: "movimiento" as const, procedencia: { fuente: "historico" as const, balance_id: a.balanceId, periodo: a.periodo, version: a.version }, coincidencia: a.coincidencia != null ? Number(a.coincidencia) : null, actualizadoEn: a.actualizadoEn.toISOString(), origenMapeo: null, actualizadoPor: null })),
    originales.map((a) => ({ ...a, fechaBalance: a.fechaBalance.toISOString(), cuenta6Russell: null, coincidencia: null, origenMapeo: null, actualizadoPor: null, actualizadoEn: null, procedencia: { fuente: "historico" as const, balance_id: a.id, periodo: a.periodo, version: a.version } })),
  );
